...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run guide_check testcases and collect sched.jsonl files."
    )
    parser.add_argument(
        "cases",
        nargs="+",
...
def inject_dump(verific_path: pathlib.Path) -> str:
    ...
    for line in lines:
        stripped = line.strip()
        if stripped.startswith("guide_check ") and "-guide-dump-sched" not in stripped:
            line = line.replace("guide_check ", "guide_check -guide-dump-sched sched.jsonl ", 1)
            injected = True
...
def run_case(case_dir: pathlib.Path, yosys_rel: str, datdir_rel: str) -> tuple[bool, str]:
    ...
    temp_verific = case_dir / "verific_sched_tmp.ys"
    temp_verific.write_text(inject_dump(verific), encoding="utf-8")
    try:
        sched = case_dir / "sched.jsonl"
        if sched.exists():
            sched.unlink()
        env = dict(os.environ)
        ...
        if not sched.exists():
            return False, "sched.jsonl not produced"
        return True, "ok"
    finally:
