...
EqGuide 支持**逐功能叠加**的验证策略，每个 feature 的 overhead 透明可度量。

riscv-mini-core 消融实验：

| 配置 | 结果 | 核算时间 | Overhead | 回退数 |
|------|------|----------|----------|--------|
| bare（模块对 CEC） | ✅ PASS | 9.3s | baseline | — |
| +partition-prove（分层） | ✅ PASS | 21.3s | +12.0s | 8 |
| +partition+shadow（安全闸） | ✅ PASS | 41.4s | +20.1s | 8 |
| +partition+shadow+slice（全功能） | ✅ PASS | 135.0s | +93.6s | 8 |

**Bare 模式最快（9.3s），Shadow 模式最安全，Slice 模式提供 A/B 对比。** 工程师可根据需求选择配置——这就是开源的力量。
